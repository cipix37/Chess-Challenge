	...
	public class ChallengeController
	{
		...
		void EndGame(GameResult result, bool log = true, bool autoStartNextBotMatch = true)
		{
			if (isPlaying)
			{
				...
				if (log)
				{
					Log($"{gameNumber} Game Over: {result} {GetPlayerName(PlayerWhite)} {GetPlayerName(PlayerBlack)}", false, ConsoleColor.Blue);
					gameNumber++;
				}
				...
				{
					BotStatsA.TotalTimeRemaining += botAPlaysWhite ? PlayerWhite.TimeRemainingMs : PlayerBlack.TimeRemainingMs;
